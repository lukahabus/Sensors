...
function getMeasurement() {
  ...
  const row = measurements[dsecs % 100];
  return {
...
function print(d1, d2, d3) {
  ...
  console.log(
    `Neighbor:\t${d2.temperature}\t\t${d2.pressure}\t\t${d2.humidity}\t\t${d2.co}\t${d2.no2}\t${d2.so2}`
  );
  console.log(
    `Parsed:\t\t${d3.temperature}\t\t${d3.pressure}\t\t${d3.humidity}\t\t${d3.co}\t${d3.no2}\t${d3.so2}`
  );
}
